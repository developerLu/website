...
<%
	...
	if (replyID != null && replyID.length() > 0) {
		pageNo = ReplyDAO.getInstance().getPageNo(topicID, replyID,
				pageRows, userinfo);
	} else {
	...
	TopicVO aTopic = TopicDAO.getInstance().getTopic(request, topicID,
			pageNo, pageRows, userinfo, baseUrl);
	if (aTopic == null) {
		request.setAttribute("errorMsg", "此主题不存在或已经被删除");
		request.getRequestDispatcher("/ejforum/error.jsp").forward(request,
				response);
		return;
	}
	...
	GroupVO userGroup = PageUtils
			.getGroupVO(userinfo, aSection, aBoard);
	if (!PageUtils.isPermitted(aBoard, userGroup,
			IConstants.PERMIT_VISIT_FORUM)
		...
		if (userinfo == null) // Guest
		{
			...
			request.getRequestDispatcher("/ejforum/login.jsp").forward(request,
					response);
			return;
		} else {
	...
	if (userGroup.groupID == 'A' || userGroup.groupID == 'M'
			|| userGroup.groupID == 'S')
		isModerator = true;
	...
	String ctxPath = request.getContextPath() + "/ejforum";
	String serverName = request.getServerName();
	if (!ctxPath.equals("/"))
		serverName = serverName + ctxPath;
	String forumStyle = PageUtils.getForumStyle(request, response,
			aBoard);

	String forumName = setting.getForumName();
	String title = PageUtils.getTitle(forumName);
	String[] menus = PageUtils.getHeaderMenu(request, userinfo);
	int maxReplies = setting.getInt(ForumSetting.FUNCTIONS,
			"maxReplies");

	sbuf.setLength(0);
	sbuf.append("./forum-").append(sectionID).append("-")
			.append(boardID).append("-1.html");
	String forumUrl = response.encodeURL(sbuf.toString());
	String homeUrl = response.encodeURL("./index.jsp");

	sbuf.setLength(0);
	sbuf.append("./post.jsp?sid=").append(sectionID).append("&fid=")
			.append(boardID);
	String postUrl = response.encodeURL(sbuf.toString());

	sbuf.setLength(0);
	sbuf.append("report.jsp?sid=").append(sectionID).append("&fid=")
			.append(boardID).append("&tid=").append(topicID)
			.append("&page=").append(pageNo);
	String reportUrl = response.encodeURL(sbuf.toString());

	ArrayList sections = cache.getSections();

	String showSectionLink = setting.getString(ForumSetting.DISPLAY,
			"showSectionLink");
	String sectionLink = null;
	if (showSectionLink.equalsIgnoreCase("yes")) {
		sbuf.setLength(0);
		sbuf.append(" &raquo;&nbsp; <A href=\"./index.jsp?sid=")
				.append(sectionID).append("\">")
				.append(aSection.sectionName).append("</A>");
...
<LINK href="styles/<%=forumStyle%>/ejforum.css" type=text/css
	rel=stylesheet>
</HEAD>
<BODY onkeydown="if(event.keyCode==27) return false;">
	...
	<DIV class=wrap>
		...
		<DIV id=foruminfo>
			<DIV id=nav>
				<A href="<%=homeUrl%>"><%=forumName%></A><%=sectionLink == null ? "" : sectionLink%>
				&raquo;&nbsp; <A href="<%=forumUrl%>"><%=aBoard.boardName%></A>
				&raquo;&nbsp;
				<%=aTopic.title%></DIV>
			<DIV id=headsearch>
	...
	frmsearch.submit();
				...
				<%-- <FORM id="frmsearch" name="frmsearch"
					...
					<INPUT type=hidden value="GB2312" name=ie> <INPUT
						...
						value=zh-CN name=hl> <INPUT type=hidden
						value="<%=serverName%>" name=sitesearch>
					...
					<a href="#" onclick="doSearch(); return false;"
						...
						src="styles/<%=forumStyle%>/images/search.gif" border="0"
						alt="站内搜索" align="absbottom" /></a>
				</FORM> --%>
			</DIV>
		</DIV>
		<DIV id=ad_text></DIV>
		<DIV class=pages_btns>
			<div class="pages">
				<a href="<%=forumUrl%>" class="next"> &lsaquo;&lsaquo; 返回主题列表</a>
			</div>
			<%
			...
			<SPAN class=postbtn id="newtopic"
				...
				<A href="<%=postUrl%>"><IMG
					src="styles/<%=forumStyle%>/images/newtopic.gif" border=0></A>
			</SPAN> <SPAN class=replybtn><A href="javascript:replyTopic();">
					<IMG alt="发表回复" src="styles/<%=forumStyle%>/images/reply.gif"
					border=0>
			</A></SPAN>
		</DIV>
		...
		<%
			...
			String showAvatar = setting.getString(ForumSetting.DISPLAY,
					"showAvatar");
			String showBrief = setting.getString(ForumSetting.DISPLAY,
					"showBrief");
			...
			if (userinfo != null
					&& userinfo.userID.equalsIgnoreCase(aTopic.userID)) {
				isTopicAuthor = true;
			...
			for (int i = 0; i < aTopic.postList.size(); i++) {
				...
				if (aPost.isHidePost == 'T') {
					...
					nickname = (aPost.u_nickname == null || aPost.u_nickname
							.length() == 0) ? userID : aPost.u_nickname;
				}

				aGroup = PageUtils.getGroupVO(userID, aPost.u_groupID,
						aPost.u_credits, moderators);
				avatar = aPost.u_avatar;
				if (avatar == null || avatar.length() == 0)
					avatar = "sample/unknown.gif";
				else if (aGroup.rights.indexOf(IConstants.PERMIT_UPLOAD_AVATAR) < 0
						&& !avatar.startsWith("sample/"))
					avatar = "sample/unknown.gif";
		...
		<DIV class="mainbox viewtopic">
			...
			<SPAN class=headactions> <A class=notabs
				href="./member/my_addfavor.jsp?tid=<%=topicID%>&fid=<%=boardID%>">收藏</A>
			</SPAN>
			<H1><%=aTopic.title%>&nbsp;<%
				...
				<TBODY>
					<TR>
						<TD class=postauthor rowspan="3"><CITE><A
								href="<%=spaceURL%><%=userID%>" target=_blank><%=nickname.length() == 0 ? "游客" : nickname%></A></CITE>
							<%
								if (showAvatar.equals("yes")) {
										int maxAvatarSize = setting.getInt(ForumSetting.MISC,
												"maxAvatarPixels");
							%>
							<DIV class=avatar>
								<A href="<%=spaceURL%><%=userID%>" target=_blank
									title="<%=nickname.length() == 0 ? "游客" : nickname%>"><IMG
									src="<%=avatarPath%><%=avatar%>" border=0
									onload="resizeImage(this, <%=maxAvatarSize%>);"></A>
							</DIV> <%
 	}
 %>
							<P>
								<EM><%=aGroup.groupName%></EM>&nbsp;
								<%
									for (int j = 0; j < count1; j++) {
								%><IMG
									alt="Rank: <%=aGroup.stars%>" src="images/star_5.gif"
									align="absmiddle" border="0">
								<%
									}
										for (int j = 0; j < count2; j++) {
								%><IMG
									alt="Rank: <%=aGroup.stars%>" src="images/star_1.gif"
									align="absmiddle" border="0">
								<%
									}
								%>
							...
							<UL>
								<LI class=friend><A
									href="./member/my_addfriend.jsp?uid=<%=userID%>"
									target=_blank>加为好友</A></LI>
								<LI class=sms><A
									href="./member/sms_compose.jsp?uid=<%=userID%>" target=_blank>发短消息</A></LI>
							</UL></TD>
						<TD class=postdetail>
							<DIV class=postinfo>
								<STRONG title="ID:<%=aPost.replyID%>"><%=seqno%><SUP>#</SUP></STRONG>
								发表于
								<%=aPost.createTime%>&nbsp;
								<%
									if (isModerator && aPost.remoteIP != null) {
											out.write("IP: ");
											out.write(aPost.remoteIP);
											out.write("&nbsp;");
										}
										if (seqno == 0 && aTopic.updateUser != null
												&& aTopic.updateUser.length() > 0) {
											out.write(" &nbsp;更新于 ");
											out.write(aTopic.updateTime);
											out.write(" &nbsp;by&nbsp; ");
											out.write(aTopic.updateUser);
											out.write("&nbsp; ");
										}
										if (replyID != null && replyID.length() > 0
												&& replyID.equals(aPost.replyID)) {
											out.write("<font color='red'>(&nbsp;回帖ID: " + replyID
													+ "&nbsp;)</font>");
										}
										if (aPost.state == 'R') {
							...
							<DIV class="postcontent">
								<%
									if (seqno == 0 && aTopic.reward > 0) {
											...
											out.write(forumStyle);
											out.write("/images/reward.gif' border='0' align='absmiddle'/> &nbsp;积分&nbsp;");
								...
								<%
										...
										if (aPost.attaches > 0 && aTopic.attachList != null) {
											...
											for (int j = 0; j < aTopic.attachList.size(); j++) {
												...
												if (!aAttach.replyID.equals(aPost.replyID))
													continue;
												attachCount++;
												if (attachCount == 1) {
													attaches = new StringBuilder(
															"<DIV class='box attachlist'><BR/>\n");
													attaches.append("<P class='attach'>附件</P>\n");
													if (!PageUtils.isPermitted(aBoard, userGroup,
															IConstants.PERMIT_DOWNLOAD)) {
														attaches.append("<EM style='padding-left:6px'>您所在的用户组无法下载或查看附件</EM>\n");
														break;
													}
												}
												if (aAttach.title == null
														|| aAttach.title.length() == 0)
													attachTitle = "";
												else
													attachTitle = "<p>" + aAttach.title + "</p>";

												attaches.append("<dl class='p_attachlist'><dt>\n")
														.append("<img src='styles/")
														.append(forumStyle)
														.append("/images/i_attach.gif' border='0' class='absmiddle'/>&nbsp;&nbsp;\n")
														.append("<a href='").append(attachURL)
														.append(aAttach.attachID)
														.append("' class='bold'>")
														.append(aAttach.filename)
														.append("</a>&nbsp;&nbsp;<em class='num'>(")
														.append(aAttach.filesize * 1.0 / 1000)
														.append(" KB)</em></dt>\n");
												attaches.append("<dd><p>").append(aAttach.createTime)
														.append(", 下载次数: <em class='num'>")
														.append(aAttach.downloads)
														.append("</em>, 下载所需积分: <em class='num'>")
														.append(aAttach.credits).append("</em></p>")
														.append(attachTitle).append("</dd></dl>\n");
											} // end for
					...
					<TR>
						<TD class=postsign>
							<%
								if (showBrief.equals("yes") && aPost.u_brief != null
											&& aPost.u_brief.trim().length() > 0) {
										out.write("<DIV class='signature'>");
										out.write(aPost.u_brief);
										out.write("</DIV>");
									}
							%>
						</TD>
						...
						<TD class=postfooter>
							<DIV class=postactions>
								<P>
									<%
										if (userinfo != null
													&& userinfo.userID.equalsIgnoreCase(aPost.u_userID)
													&& !AppUtils.isBeforeDate(aPost.createTime,
															cal.getTime()))
												isAuthor = true;
											else
												isAuthor = false;
											...
											if (isAuthor) {
												...
												if (seqno == 0)
													...
													userPostIDs.append('r').append(aPost.replyID)
															.append(',');
											}

											if (seqno == 0
													&& (isAuthor || userGroup.rights
															.indexOf(IConstants.PERMIT_MOVE_POST) >= 0)) {
												isManager = true;
									%>
									...
									<%
											...
											if (isAuthor
													|| userGroup.rights
															.indexOf(IConstants.PERMIT_EDIT_POST) >= 0) {
												isManager = true;
									%>
									...
									%>
									<a
										href="javascript:doManage('delete', '<%=aPost.replyID%>');">删除</a>&nbsp;&nbsp;
									<%
										}
											if (aTopic.state != 'C') {
									%>
									...
									%>
									<a
										href="javascript:doQuote('<%=seqno%>','<%=nickname%>','<%=aPost.createTime%>');">引用</a>&nbsp;&nbsp;
									...
									%>
									<a
										href="javascript:doManage('setbest', '<%=aPost.replyID%>');">最佳回复</a>&nbsp;&nbsp;
									<%
										}
									%>
									<SPAN title="顶部" class="scrolltop" onclick="scroll(0,0)">TOP</SPAN>
								</P>
							</DIV>
						</TD>
					</TR>
...
var isClosed = <%=aTopic.state == 'C'
					? "true"
					: (aTopic.replies >= maxReplies ? "true" : "false")%>;
var isDigest = <%=aTopic.isDigest == 'T' ? "true" : "false"%>;
<%if (isManager) {
				sbuf.setLength(0);
				sbuf.append("manage.jsp?sid=").append(sectionID)
						.append("&fid=").append(boardID).append("&chkTopicID=")
						.append(topicID).append("&page=").append(pageNo);%>
...
<%}%>
		...
		<DIV class=legend id=footfilter>
			<DIV class=jump_sort>
				<form id="frmsort" name="frmsort" action="<%=forumUrl%>"
					method="post">
					<SELECT
						...
						<%
							if (sections != null) {
								...
								for (int i = 0; i < sections.size(); i++) {
									...
									if (tmpSection.boardList == null)
										continue;

									sb.append("<OPTGROUP label=\"")
											.append(tmpSection.sectionName).append("\">\n");

									for (int j = 0; j < tmpSection.boardList.size(); j++) {
										tmpBoard = (BoardVO) tmpSection.boardList.get(j);
										if (tmpBoard.state == 'I' && !isModerator)
											continue;
										sbuf.setLength(0);
										sbuf.append("./forum-").append(tmpSection.sectionID)
												.append("-").append(tmpBoard.boardID)
												.append("-1.html");
										tmpUrl = response.encodeURL(sbuf.toString());
										sb.append("<OPTION value=\"").append(tmpUrl)
												.append("\">&nbsp; &gt; ")
												.append(tmpBoard.boardName)
												.append("</OPTION>\n");
									}
									sb.append("</OPTGROUP>");
								}
				...
				<form id="frmpost" name="frmpost" action="<%=postUrl%>"
					method="post">
					<INPUT type=hidden
						value="<%=aTopic.title.replace("\"", "&quot;")%>" name="topic">
					<INPUT type=hidden value="" name="subject"> <INPUT
						type=hidden value="" name="content"> <INPUT type=hidden
						value="<%=aTopic.isReplyNotice%>" name="notice">
				</form>
			</DIV>
		</DIV>
	</DIV>
