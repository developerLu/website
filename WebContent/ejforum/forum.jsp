...
<%
	...
	if (sectionID == null) {
		...
		if (tmpBoard != null)
			...
			request.getRequestDispatcher("/ejforum/error.jsp").forward(
					request, response);
			return;
		}
	}
	...
	if (!PageUtils.isPermitted(aBoard, userGroup,
		...
		if (userinfo == null) // Guest
		{
			...
			request.getRequestDispatcher("/ejforum/login.jsp").forward(
					request, response);
			return;
		} else {
	...
	int pageRows = setting
			.getInt(ForumSetting.DISPLAY, "topicsPerPage");
	...
	System.out.println("forumStyle");
	String rssStyle = setting.getString(ForumSetting.FUNCTIONS,
			"RssStyle");
	String feedUrl = null;
	...
	Object[] result = TopicDAO.getInstance().getTopicList(aSection,
			aBoard, sortField, spec, baseUrl, pageNo, pageRows);
%>
...
<%=PageUtils
					.getRSSLink(request, forumName, aSection, aBoard)%>
	...
	<DIV class=wrap>
		...
		<DIV id=foruminfo>
			<DIV id=nav>
				...
				<P>
					版主:
					<%=moderatorLink == null ? "空缺中" : moderatorLink%></P>
			</DIV>
			<DIV id=headsearch>
	...
	frmsearch.submit();
		...
		<DIV class="mainbox forumlist" style="padding-top: 0px">
			<TABLE cellSpacing=0 cellPadding=0>
				<TBODY class="info">
					<TR>
						<TD class="subject">本版规则</TD>
					</TR>
					<TR>
						<TD><%=aBoard.ruleCode%></TD>
					</TR>
				</TBODY>
			</TABLE>
		</DIV>
		...
		<DIV class=pages_btns>
			...
			<SPAN class=postbtn id="newtopic"
				...
				<A href="post.jsp?sid=<%=sectionID%>&fid=<%=boardID%>"><IMG
					alt=发新话题 src="styles/<%=forumStyle%>/images/newtopic.gif"
					border=0></A>
			</SPAN>
		</DIV>
		<UL class="popmenu_popup newtopicmenu" id="newtopic_menu"
			...
			<LI class="reward"><a
				href="post.jsp?sid=<%=sectionID%>&fid=<%=boardID%>&act=reward">发布悬赏</a></LI>
		</UL>
		<DIV id=headfilter>
			<UL class=tabs>
				<LI <%=spec.equals("all") ? " class=spec" : ""%>><A
					href="<%=forumUrl%>">全部</A></LI>
				...
				<%-- <LI style="border: none"><A title="RSS Feed"
					href="<%=feedUrl%>" target=_blank> <IMG alt="RSS Feed"
						src="images/rss.gif" border="0"></A></LI> --%>
			</UL>
		</DIV>
		<DIV class="mainbox topiclist">
			<H1>
				<A class=bold href="<%=forumUrl%>"><%=aBoard.boardName%></A>
			</H1>
			<%
				if (isModerator) {
					sbuf.setLength(0);
					sbuf.append("manage.jsp?sid=").append(sectionID)
							.append("&fid=").append(boardID).append("&page=")
							.append(pageNo);
			%>
			<FORM name="frmmanage"
				action="<%=response.encodeURL(sbuf.toString())%>" method=post>
				<%
					}
				%>
				<TABLE cellSpacing=0 cellPadding=0>
					...
					<%
						if (result != null && result[1] != null) {
							...
							int hotTopicPosts = setting.getInt(ForumSetting.DISPLAY,
									"hotTopicPosts");
							int hotTopicVisits = setting.getPInt(ForumSetting.DISPLAY,
									"hotTopicVisits", 100);
				...
				<TABLE cellSpacing=0 cellPadding=0>
					...
					<%
								...
								lastPostUrl = response.encodeURL(sbuf.toString()
										+ "-999.html");
								...
								if (aTopic.isHidePost == 'T') {
									...
									nickname = (aTopic.nickname == null || aTopic.nickname
											.length() == 0) ? userID : aTopic.nickname;
								}
								lastNickname = (aTopic.lastNickname == null || aTopic.lastNickname
										.length() == 0)
										? aTopic.lastPostUser
										: aTopic.lastNickname;
					...
					<TBODY>
						<TR>
							<TD class=folder><A title=新窗口打开 href='<%=topicUrl%>'
								target=_blank><IMG src="images/<%=topicIcon%>"></A></TD>
							<TD>
								<%
									if (aTopic.isDigest == 'T') {
								%> <LABEL><IMG alt="精华" src="images/digest.gif"
									align="absmiddle">&nbsp;</LABEL> <%
 	}
 			if (aTopic.topScope != 'N') {
 				hasTopTopics = true;
 %> <LABEL><IMG
									alt="<%=aTopic.topScope == '1'
								? "全局置顶"
								: (aTopic.topScope == '2' ? "分区置顶" : "本版置顶")%>"
									src="images/top_<%=aTopic.topScope%>.gif" align="absmiddle">&nbsp;</LABEL>
								<%
									}
											if (isModerator) {
								%> <input class="checkbox" type="checkbox" name="chkTopicID"
								value="<%=aTopic.topicID%>" /> <%
 	}
 			String highColor = null;
 			if (aTopic.highColor != null
 					&& aTopic.highColor.length() > 0) {
 				highColor = " style=\"color:" + aTopic.highColor + "\"";
 			}
 %> <A href='<%=topicUrl%>'
								<%if (highColor != null)
						out.write(highColor);%>><%=aTopic.title%></A>
								<%
							...
							<TD class=author><CITE><A
									href="<%=spaceURL%><%=userID%>"><%=nickname.length() == 0 ? "游客" : nickname%></A>
							...
							<TD class=lastpost><EM><A href="<%=lastPostUrl%>"><%=aTopic.lastPostTime%></A></EM>
								<CITE>by <A
									href="<%=spaceURL%><%=aTopic.lastPostUser%>"><%=lastNickname.length() == 0 ? "游客" : lastNickname%></A>
							</CITE></TD>
						</TR>
					</TBODY>
					<%
					...
					<tbody>
						<tr>
							<td class=folder>&nbsp;</td>
							<td colspan="4">本版块或指定的范围内尚无主题。</td>
						</tr>
					</tbody>
					<%
						}
	...
	function doManage(action) 
	{
		...
		<DIV class=pages_btns>
			...
			<SPAN class=postbtn id="newtopictmp"
				onmouseover="$('newtopic').id = 'newtopictmp';this.id = 'newtopic';showMenu(this.id);">
				<A href="post.jsp?sid=<%=sectionID%>&fid=<%=boardID%>"><IMG
					alt=发新话题 src="styles/<%=forumStyle%>/images/newtopic.gif"></A>
			</SPAN>
		</DIV>
			...
			<DIV class=jump_sort>
				<form id="frmsort" name="frmsort" action="<%=forumUrl%>"
					...
					<SELECT
						...
						<OPTION value="A_visits">按浏览次数排序(升序)</OPTION>
					</SELECT> <INPUT type=hidden value="<%=spec%>" name="spec">
				</form>
			</DIV>
			<DIV>
				<LABEL><IMG alt=正常主题 src="images/folder_common.gif">正常主题</LABEL>
				<LABEL><IMG alt=热门主题 src="images/folder_hot.gif">热门主题</LABEL>
				<LABEL><IMG alt=关闭主题 src="images/folder_lock.gif">关闭主题</LABEL>
			</DIV>
		</DIV>
		<SCRIPT type=text/javascript>
