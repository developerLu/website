...
public class StringUtils {
	 ...
	 * @return String
	 */
	public static String replaceFileSuffix(String uri, String suffix) {
	 ...
	 * @return
	 */
	public static String getFileSuffix(String uri) {
		if(uri == null) return null;
		int pos = uri.lastIndexOf("?");
		if (pos > -1) {
			//如果包含参数，则去除参数
			return getFileSuffix(uri.substring(0, pos));
		} else {
			pos = uri.lastIndexOf("/");
			if (pos > -1) {
				//如果包含路径，则去除路径
				return getFileSuffix(uri.substring(pos+1));
			} else {
				pos = uri.lastIndexOf(".");
				if (pos > -1) {
	...
	public static boolean isLetter(String str) {
		if (StringUtils.isEmpty(str))
			return false;
		if (str.replaceAll("\\s*", "").matches("^[a-zA-Z]*"))
			return true;
		return false;
	}
	...
	public static Integer mapLen(Map<?, ?> map) {
		if (StringUtils.isNotEmptyMap(map))
			return map.size();
		return 0;
	}
	...
	public static void main(String[] args) throws IOException {
		// System.out.println(convertNumber("1.555"));

		// String str =
		// "<a href=${fn:getLink('catalog/checkmark1.jsp')}>1</a><a href=${fn:getLink('catalog/checkmark2.jsp')}>2</a><a href=${fn:getLink('catalog/checkmark3.jsp')}>3</a>";
		// String[] s = str.split("\\$\\{fn\\:getLink\\(\\'");
		// StringBuffer sb = new StringBuffer("");
		// for(int i=0;i<s.length;i++){
		// String temp = s[i];
		// if(temp.contains(")}")){
		...
		// StringTokenizer(str,"//$//{fn:getLink//(//'");
		// String sss = "";
